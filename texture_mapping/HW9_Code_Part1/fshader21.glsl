...
void main()
{
		...
		vec4 diffuse = dTerm*texture2D( texture, texCoord );
		vec4 diffuse1 = dTerm1*texture2D( texture, texCoord );
		...
		if(color_flag == 1)
			temp_color = color_data_to_fshader;
		else
			temp_color = ambient + diffuse + specular + ambient1 + diffuse1 + specular1;	
